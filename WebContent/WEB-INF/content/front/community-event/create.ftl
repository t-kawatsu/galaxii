...
<section>
  ...
  <form action="${url('/community-event/create/' + community.id)}" method="POST" class="ce-form">
	<div class="table">
	  <div class="table-row <@my.errorInputClass 'communityEventForm.title'/>">
	    <@my.activityIcon communityContentsCategory('event') />
	    <@s.textarea name="communityEventForm.title" cssClass="ct-title form-text auto-resize-textbox input-shadow cancel-enter" placeholder="イベント名を入力してください" />
	    <@s.fielderror><@s.param value="%{'communityEventForm.title'}" /></@s.fielderror>
	  </div>
	</div>
	<div class="table ce-datetimepicker-table">
	  <div class="table-row">
		<div class="table-cell">
			<@s.textfield name="communityEventForm.startAtDate" cssClass="form-text input-shadow datepicker" placeholder="${date?string('yyyy/MM/dd')}"/>
		</div>
		<div class="table-cell">
		    <@s.textfield name="communityEventForm.startAtTime" cssClass="form-text input-shadow timepicker" placeholder="${date?string('H:30')}" />
		</div>
		...
		<div class="table-cell ce-end-at-items" <#if !communityEventForm.endAt??>style="display:none"</#if> >
			<@s.textfield name="communityEventForm.endAtDate" cssClass="form-text input-shadow datepicker" placeholder="${date?string('yyyy/MM/dd')}"/>
		</div>
		<div class="table-cell ce-end-at-items" <#if !communityEventForm.endAt??>style="display:none"</#if> >
		    <@s.textfield name="communityEventForm.endAtTime" cssClass="form-text input-shadow timepicker" placeholder="${date?string('H:30')}" />
		</div>
		<div class="table-cell community-event-add-end-at">
		  <button type="button" class="form-button btn-community-event-add-end-at btn-a btn-small c-inline-btn " style="margin:0px 6px;">
	...
	<div class="table">
	  <div class="table-row">
		<div class="table-cell <@my.errorInputClass 'communityEventForm.place'/>" >
	      <div class="pos-r">
